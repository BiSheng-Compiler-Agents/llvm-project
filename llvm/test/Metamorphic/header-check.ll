; RUN: rm -f -- %1 && opt -passes='default<O3>' --passes='protean-collect-features' --enable-protean-feature-dump  --protean-dump-file=%1 --debug-only=proteanFC < %s 2> %2
; RUN: sed '1d' %1 > %3
; RUN: cat %3 %2 | FileCheck %s
