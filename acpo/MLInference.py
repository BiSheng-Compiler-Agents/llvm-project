...
def create_MLInference(model_inference, model_dir, infer, output_key, classes_dict,
  ...
  elif "IR2ScoreInference" in model_inference:
    from IR2ScoreInference import IR2ScoreInference
    return IR2ScoreInference(model_dir, infer, output_key, classes_dict, output_name)
  else:
...
################################################################################
################################################################################
# Hardcoded values for doing an inference test
#plu_path = os.path.join(os.path.dirname(__file__), './models/plu-dummy')
################################################################################
def set_features_sample_1():
  """
  Recieve the features from LLVM and pass it to inference()
  FIXME: For now it is hardcoded.
  """
  # 10 features reqired by the plu-dummy model
...
def set_features_sample():
  """
  Recieve the features from LLVM and pass it to inference()
  FIXME: For now it is hardcoded.
  """
  # 22 features reqired by the plu-dummy model
  #df = ['main_for.body231_1.txt','main','for.body231','300','400','0','150','0','8','4294967295','4294967295','2','0','0','1','0','0','0','0','0','60','10','0','492','0','partial']
  df = [
      '300', '400', '0', '150', '0', '8', '4294967295', '4294967295', '2', '0',
      '0', '1', '0', '0', '0', '0', '0', '60', '10', '0', '492'
  ]
  input = np.array(df, dtype=np.float32)
  input = input.reshape(1, 21)
  return input
...
def inference_sample():
  ...
  """
  input = []
  # FIXME: Calling the load_model() generates a wierd error,
  #infer = load_model(frozen_model)
  # inlined the function here instead
  imported = tf.saved_model.load(frozen_model)
  infer = imported.signatures["action"]
  ACPO_LOG("Frozen model successfully load.")
  input = set_features_sample()
  ACPO_LOG("Features successfully fetched, FT size: ", np.shape(input),
        input.dtype)
  output = infer(tf.constant(input))
  output = output['unrolling_decision'].numpy()
  ACPO_LOG(output)
